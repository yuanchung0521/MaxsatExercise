        ...
        for i1 in range(len(V)):
            ...
            for j1 in range(len(V[i])):
                ...
                for k in range(len(V[i][j])):
                    ...
                    ans = 0
                    for h in range(V[i][j][k].adj_num):
                        ans += int(value_list[FLOW[E_list[i][j][k][h]][n][m]])
                        '''if variable_true[FLOW[E_list[i][j][k][h]][n][m]]:
                            if model[variable_list[FLOW[E_list[i][j][k][h]][n][m]]] == 1:
                                ans += 1
                        else:
                            if value_list[FLOW[E_list[i][j][k][h]][n][m]] == 1:
                                ans += 1'''
...
for n in range(len(net_name)):
    ...
    for i1 in range(len(V)):
        ...
        for j1 in range(len(V[i])):
            ...
            for k in range(len(V[i][j])):
                #k -= 1
                #if value_list[VN[i][j][k][n]]:
                #    print([i, j, k, n, VN[i][j][k][n]])
                ans = 0
                '''if variable_true[VN[i][j][k][n]]:
                    if model[variable_list[VN[i][j][k][n]]] == 1:
                        ans += 1
                else:
                    if value_list[VN[i][j][k][n]] == 1:
                        ans += 1'''
                ans += int(value_list[VN[i][j][k][n]])
    ...
    for i1 in range(len(V)):
        ...
        for j1 in range(len(V[i])):
            ...
            for k in range(len(V[i][j])):
                ...
                ans = 0
                for h in range(V[i][j][k].adj_num):
                    ans += int(value_list[EN[E_list[i][j][k][h]][n]])
                    '''if variable_true[FLOW[E_list[i][j][k][h]][n][m]]:
                        if model[variable_list[FLOW[E_list[i][j][k][h]][n][m]]] == 1:
                            ans += 1
                    else:
...
for i1 in range(len(V)):
    ...
    for j1 in range(len(V[i])):
        ...
        for k in range(len(V[i][j])):
            ...
            ans = 0
            for h in range(V[i][j][k].adj_num):
                ans += int(value_list[M[E_list[i][j][k][h]]])
                '''
                if variable_true[FLOW[E_list[i][j][k][h]][n][m]]:
                    if model[variable_list[FLOW[E_list[i][j][k][h]][n][m]]] == 1:
                        ans += 1
